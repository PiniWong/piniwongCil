...
const createprojectAction = async (project)=>{
    ...
    console.log('piniwong helps you create your project, please wait a moment~')

    //1、clone项目
    //使用git clone的方式
    await download(vueRepo,project,{clone:true},async() => {
...
const addBaseRAction = async(name,title,dest)=>{
    ...
    //view下的vue
    let vuetargetDest = null
    const vuetargetdir_arr = [
        path.resolve('./',`src/views`),
        path.resolve('./',`src/view`),
        path.resolve('./',`src/pages`),
        path.resolve('./',`src/page`)
    ]
    vuetargetdir_arr.forEach(el => {
        if(fs.existsSync(el)){
            vuetargetDest = path.resolve('./',`${el}/${name}`)
            return
        }
    });
...
const  addPageChildRAction = async(name,title,routername,dest)=>{
    ...
    console.log(vuetargetdir_arr + '数组')
    vuetargetdir_arr.forEach(el => {
        if(fs.existsSync(el)){
            vuetargetDest = path.resolve('./',`${el}/${name}`)
            return
